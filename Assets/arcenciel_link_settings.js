class ArcEnCielLinkSettingsUI {
  ...
  register() {
    ...
    inputs.innerHTML =
      makeTextInput(
        ...
        "",
        "Base URL",
        "ArcEnCiel Link API endpoint.",
        "",
        "normal",
        "https://link.arcenciel.io/api/link",
        false,
        true,
      ) +
      makeTextInput(
        null,
        "arcenciel_link_link_key",
        "",
        "Link Key",
        "Link key used for verification (starts with lk_).",
        "",
        "secret",
        "lk_...",
        false,
      ...
      makeCheckboxInput(
        ...
        "",
        "Enable Link Worker",
        "Enable the ArcEnCiel Link worker connection.",
        false,
        false,
        true,
      ) +
      makeNumberInput(
        null,
        "arcenciel_link_min_free_mb",
        "",
        "Min Free Disk (MB)",
        "Minimum free disk space required to accept downloads.",
        2048,
        0,
        1000000,
        1,
      ...
      makeNumberInput(
        null,
        "arcenciel_link_max_retries",
        "",
        "Max Retries",
        "Maximum download retries per job.",
        5,
        1,
        20,
        1,
      ...
      makeCheckboxInput(
        null,
        "arcenciel_link_save_html",
        "",
        "Save HTML Previews",
        "Save HTML preview sidecars when available.",
        false,
        false,
        true,
      ) +
      makeCheckboxInput(
        null,
        "arcenciel_link_allow_private",
        "",
        "Allow Private Origins",
        "Allow private/localhost origins for local development.",
        false,
        false,
        true,
      );

    getRequiredElementById("arcenciel_link_settings_save").addEventListener(
      "click",
      () => this.save(),
    );
    getRequiredElementById("arcenciel_link_settings_reload").addEventListener(
      "click",
      () => this.load(),
    );
  ...
  setMeta(message) {
    getRequiredElementById("arcenciel_link_settings_meta").textContent =
      message || "";
  }
  ...
  applyState(data) {
    ...
    getRequiredElementById("arcenciel_link_base_url").value =
      data.baseUrl || "";
    getRequiredElementById("arcenciel_link_enabled").checked = !!data.enabled;
    getRequiredElementById("arcenciel_link_min_free_mb").value =
      data.minFreeMb ?? 2048;
    getRequiredElementById("arcenciel_link_max_retries").value =
      data.maxRetries ?? 5;
    getRequiredElementById("arcenciel_link_backoff_base").value =
      data.backoffBase ?? 2;
    getRequiredElementById("arcenciel_link_save_html").checked =
      !!data.saveHtmlPreview;
    getRequiredElementById("arcenciel_link_allow_private").checked =
      !!data.allowPrivateOrigins;

    const linkKeyInput = getRequiredElementById("arcenciel_link_link_key");
    linkKeyInput.value = "";
    linkKeyInput.placeholder = data.linkKeySet
  ...
  async save() {
    ...
    const baseUrl = getRequiredElementById(
      "arcenciel_link_base_url",
    ).value.trim();
    if (!baseUrl) {
    ...
    const linkKey = getRequiredElementById(
      "arcenciel_link_link_key",
    ).value.trim();

    if (enabled && !linkKey && !this.state.linkKeySet) {
      this.setStatus("Link Key required to enable.", true);
      return;
    }

    const minFreeMb = parseInt(
      getRequiredElementById("arcenciel_link_min_free_mb").value,
      10,
    );
    ...
    const payload = {
      ...
      saveHtmlPreview: getRequiredElementById("arcenciel_link_save_html")
        .checked,
      allowPrivateOrigins: getRequiredElementById(
        "arcenciel_link_allow_private",
      ).checked,
    };
