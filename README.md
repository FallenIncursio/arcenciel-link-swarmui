...
In SwarmUI, open the Arc en Ciel Link panel's inbox. Drafts fill supported parameter controls and retain their
previous values. Disable Generate Forever and live generation previews before reviewing or applying a draft.
Native catalog recognition does not guarantee that every configured remote generation backend can load a model.
